...
export class UnitOfMeasureService {
  async list(tenant: TenantContext, params: ListParams) {
    ...
    const query = unitOfMeasureRepository
  ...
  async show(tenant: TenantContext, id: number) {
    const row = await unitOfMeasureRepository.findById(tenant.company.id, id)
    if (!row) throw new NotFoundException('Unidade de medida não encontrada.')
    return this.serialize(row)
  }

  async create(tenant: TenantContext, dto: CreateUnitOfMeasureDTO) {
    const row = await UnitOfMeasure.create({
  ...
  async update(tenant: TenantContext, id: number, dto: UpdateUnitOfMeasureDTO) {
    const row = await unitOfMeasureRepository.findById(tenant.company.id, id)
    if (!row) throw new NotFoundException('Unidade de medida não encontrada.')
    ...
    try {
      ...
      if (isForeignKeyViolation(error)) {
        throw new ConflictException(
          'Não é possível excluir esta unidade de medida porque está em uso.'
        )
      }
      throw error
    }
  ...
  private serialize(row: UnitOfMeasure) {
    return {
      id: row.id,
