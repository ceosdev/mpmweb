...
export const createExpenseGroupValidator = vine.compile(
  vine.object({
    description: vine.string().trim().minLength(1).maxLength(120),
    isActive: vine.boolean().optional(),
  })
)
createExpenseGroupValidator.messagesProvider = messages

export const updateExpenseGroupValidator = vine.compile(
  vine.object({
    description: vine.string().trim().minLength(1).maxLength(120).optional(),
    isActive: vine.boolean().optional(),
  })
)
updateExpenseGroupValidator.messagesProvider = messages
