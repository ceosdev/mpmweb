...
/**
 * Pagamento em lote: escolhe **uma** forma de pagamento e baixa todos os títulos
 * selecionados na data de hoje, cada um pelo saldo restante. O backend deriva
 * valor e data — aqui só enviamos ids + tipo. Ver
 * `docs/spec/financeiro/003-pagamento-em-lote.md`.
 */
export function BatchPaymentDialog({
  ...
  const totalToPay = useMemo(
    () => payables.reduce((sum, row) => sum + row.balance, 0),
    [payables]
  )
  ...
  const mutation = useMutation({
    mutationFn: () =>
      payablesApi.batchSettle(
        payables.map((row) => row.id),
        Number(paymentTypeId)
      ),
    onSuccess: (result) => {
      toast.success(
        `${result.settledCount} ${result.settledCount === 1 ? 'título pago' : 'títulos pagos'} em lote.`
      )
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Pagar títulos em lote</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {/* Destaque do modal: o aviso pedido, com o resumo do que será pago. */}
