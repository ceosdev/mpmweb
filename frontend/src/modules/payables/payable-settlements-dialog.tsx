...
import {
  payableSettlementsApi,
  type CreatePayableSettlementPayload,
  type UpdatePayableSettlementPayload,
} from '@/services/payable-settlements-api'
...
interface PayableSettlementsDialogProps {
  open: boolean
...
/**
 * Baixas (pagamentos) de um título — CRUD aberto a partir do grid de contas a
 * pagar. Abre na **listagem**; "Nova baixa" troca a mesma janela para o
 * formulário (sem empilhar modais). Toda baixa criada/editada/removida atualiza
 * o saldo e o status do título — o backend recalcula, e aqui só revalidamos as
 * queries. Ver `docs/spec/financeiro/002-baixa-de-titulo.md`.
 */
export function PayableSettlementsDialog({
  open,
  onOpenChange,
  payable,
}: PayableSettlementsDialogProps) {
  const { tenant } = useAuth()
  const queryClient = useQueryClient()
  const companyId = tenant?.companyId
  const payableId = payable?.id ?? null

  const [view, setView] = useState<'list' | 'form'>('list')
  const [editing, setEditing] = useState<PayableSettlement | null>(null)
  ...
  const detailQuery = useQuery({
    queryKey: ['payables', companyId, 'detail', payableId],
    queryFn: () => payablesApi.get(payableId as number),
    enabled: open && payableId !== null,
    initialData: payable ?? undefined,
  })
  const current = detailQuery.data ?? payable

  const listQuery = useQuery({
    queryKey: ['payable-settlements', companyId, payableId],
    queryFn: () => payableSettlementsApi.list(payableId as number),
    enabled: open && payableId !== null,
  })

  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      payableSettlementsApi.remove(payableId as number, id),
    onSuccess: () => {
  ...
  function invalidate() {
    // Uma invalidação em 'payables' cobre a lista do grid E o detalhe do título.
  ...
  function openEdit(row: PayableSettlement) {
    setEditing(row)
    setView('form')
  }
  function backToList() {
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {current && <PayableStatusBadge status={current.status} />}
          </div>
        </DialogHeader>
        ...
        {current && (
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 rounded-lg border bg-muted/40 px-4 py-3 text-sm sm:grid-cols-4">
            ...
            <HeaderItem label="Cedente">{current.supplierName || '—'}</HeaderItem>
            <HeaderItem label="Total">{formatCurrency(current.total)}</HeaderItem>
            <HeaderItem label="Pago">{formatCurrency(current.paidAmount)}</HeaderItem>
          ...
          <>
            {listQuery.isLoading ? (
              ...
              <EmptyState
                ...
                description="Registre o primeiro pagamento deste título."
              />
            ) : (
              <Table>
                <TableHeader>
                    ...
                    <TableRow key={row.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Can permission="payable_settlements.edit">
                            <Button
                              variant="ghost"
                              size="icon"
                          ...
                          <Can permission="payable_settlements.delete">
                            <Button
                              variant="ghost"
                              size="icon"
            ...
            <DialogFooter className="sm:justify-between">
              ...
              <Can permission="payable_settlements.create">
...
const formSchema = z.object({
  ...
  /** Centavos crus. */
  amount: z.string().min(1, 'Valor pago é obrigatório.'),
  documentNumber: z
    .string()
    .trim()
...
interface SettlementFormProps {
  payable: Payable
  settlement: PayableSettlement | null
  onCancel: () => void
  onSaved: () => void
}
 ...
 * Valor pago já vem preenchido com o **saldo restante** — o caso mais comum é a
 * baixa total.
 */
function SettlementForm({ payable, settlement, onCancel, onSaved }: SettlementFormProps) {
  ...
  const form = useForm<SettlementFormValues>({
    ...
    defaultValues: {
      ...
      // Default = saldo restante (baixa total num clique); na edição, o valor da baixa.
      amount: reaisToCents(settlement ? settlement.amount : payable.balance),
  ...
  const mutation = useMutation({
    mutationFn: (values: SettlementFormValues) => {
      const payload: CreatePayableSettlementPayload = {
      ...
      if (isEdit && settlement) {
        return payableSettlementsApi.update(
          payable.id,
          settlement.id,
          payload satisfies UpdatePayableSettlementPayload
        )
      }
      return payableSettlementsApi.create(payable.id, payload)
    },
    onSuccess: () => {
