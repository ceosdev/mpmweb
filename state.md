...
- **RBAC** — catálogo em `backend/app/abilities/catalog.ts`. Slugs `dashboard.*`, `companies.*`, `users.*`, `permissions.*`, `roles.*`, `products.*`, `product_assets.*`, `brands.*`, `brand_models.*`, `payables.*`, `payable_settlements.*`, entre outros. ROOT bypassa tudo. ADMIN/OPERADOR não existem mais como perfis seedados — cada empresa cria os seus.
